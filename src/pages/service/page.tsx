  ...
  if (!service) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 px-4">
        <p className="text-muted-foreground font-sans">Услуга не найдена</p>
        <Link to="/" className="text-primary underline font-sans text-sm">На главную</Link>
      </div>
    );
  }
            ...
            <div className="py-2">
              ...
              {service.details && service.details.length > 0 && (
                <div className="mb-8">
                  ...
                  {service.recommended && (
                    <p className="mt-4 text-xs text-muted-foreground font-sans font-light">{service.recommended}</p>
                  )}
                </div>
              )}
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              ...
              <a href={TG_LINK} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 bg-white text-foreground px-7 py-3 rounded-full text-xs font-sans tracking-wider uppercase hover:bg-white/90 transition-all cursor-pointer">
                <CalendarCheck className="w-3.5 h-3.5" /> Записаться в Telegram
              </a>
            </div>
          </motion.div>
        </div>
