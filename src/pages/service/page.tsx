...
import { massageServices, saunaServices, TEL_HREF, TG_LINK, LOGO_URL } from "@/lib/services-data.ts";
    ...
    <div className="min-h-screen bg-background">
      <nav className="fixed top-0 left-0 right-0 z-50 bg-background/95 backdrop-blur-md border-b border-border">
        <div className="max-w-6xl mx-auto px-5 py-3.5 flex items-center justify-between gap-4">
          <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-xs text-muted-foreground hover:text-foreground transition-colors cursor-pointer font-sans tracking-wider uppercase">
            <ArrowLeft className="w-4 h-4" />{t("service.page.back")}
          </button>
      ...
      <div className="pt-14">
        <div className="max-w-6xl mx-auto px-5 py-14 md:py-20">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: [0.25, 0.1, 0.25, 1] as const }} className="grid md:grid-cols-2 gap-10 md:gap-16 items-start mb-16">
            <div className="rounded-2xl overflow-hidden aspect-[3/4] md:aspect-[4/5]">
              <motion.img src={service.img} alt={title} className="w-full h-full object-cover" initial={{ scale: 1.06 }} animate={{ scale: 1 }} transition={{ duration: 1.4, ease: "easeOut" } as const} />
            </div>
            <div className="py-2">
              {service.tag && (
                  ...
                  <ul className="space-y-2">
                    {detailKeys.map((d, i) => (
                      <motion.li key={i} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 + i * 0.07 } as const} className="flex items-start gap-2.5 text-sm font-sans text-foreground/70">
                        <CheckCircle2 className="w-3.5 h-3.5 text-primary mt-0.5 shrink-0" />{d}
                      </motion.li>
                    ))}
                  </ul>
                  {recommended && <p className="mt-4 text-xs text-muted-foreground font-sans font-light">{recommended}</p>}
                </div>
              )}
                ...
                <motion.div key={s.key} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + i * 0.06 } as const}>
                  <Link to={`/service/${s.key}`} className="group block cursor-pointer">
                    ...
                    <p className="text-muted-foreground text-[11px] font-sans">
                      {s.prices.length > 0 ? s.prices.map((p) => `${p.label} // ${p.price}`).join(" · ") : t(`service.${s.key}.subtitle`)}
                    </p>
                  </Link>
                </motion.div>
              ))}
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 } as const} className="mt-16 bg-primary text-primary-foreground rounded-2xl p-10 text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <a href={TEL_HREF} className="flex items-center justify-center gap-2 border border-white/35 text-white px-7 py-3 rounded-full text-xs font-sans tracking-wider uppercase hover:bg-white/10 transition-all cursor-pointer"><Phone className="w-3.5 h-3.5" /> {t("call.btn")}</a>
              <a href={TG_LINK} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 bg-white text-neutral-900 px-7 py-3 rounded-full text-xs font-sans tracking-wider uppercase hover:bg-white/90 transition-all cursor-pointer"><CalendarCheck className="w-3.5 h-3.5" /> {t("service.page.book_online")}</a>
            </div>
          </motion.div>
        </div>
      </div>
